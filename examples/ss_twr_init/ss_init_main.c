  ...
  {		
    ...
    * As the sequence number field of the frame is not relevant, it is cleared to simplify the validation of the frame. */
    ...
    {	
      ...
      /* Using READ DIAGNOSTICS */

      dwt_rxdiag_t diag;
      dwt_readdiagnostics(&diag); // I think it should work here, given that isr() constant is mentioned in the earlier while loop

      //float SNR;

      // Because we use DWT_PRF_64M,      /* Pulse repetition frequency. */ in our config
      float PRF = 121.74;
      //float N_sq = pow((double)diag.rxPreamCount, 2.0);
      //double first_path_power_dBm = 10 * log10(
      
      //  (pow((double)diag.firstPathAmp1, 2.0) + pow((double)diag.firstPathAmp2, 2.0) + pow((double)diag.firstPathAmp3, 2.0)) / N_sq
      
      //) - PRF;


      //double RX_power_dBm = 10 * log10( ((double)diag.maxGrowthCIR * POW_2_17) / N_sq) - PRF;
      //float NLoS_score = fabs(first_path_power_dBm - RX_power_dBm);
      //bool is_NLoS = NLoS_score > 6.0;

      // TODO: Back this and all of your scripts up to a repo

      tof = ((rtd_init - rtd_resp * (1.0f - clockOffsetRatio)) / 2.0f) * DWT_TIME_UNITS; // Specifying 1.0f and 2.0f are floats to clear warning 
      distance = tof * SPEED_OF_LIGHT;

      // Dump whole struct + distance to out
      printf("%f, %u, %u, %u, %u, %u, %u, %u, %u, %f\r\n", distance, diag.maxNoise, diag.firstPathAmp1, diag.stdNoise, diag.firstPathAmp2, diag.firstPathAmp3, diag.maxGrowthCIR, diag.rxPreamCount, diag.firstPath, PRF);
      




      /* Directly Reading From Registers */

      // Note: See section 4.3.1 of user manual for enabling double buffered operation, this is the only way to get the register querying to work?
      // Seems buffers have different values at different points, i.e. first 2 var, clear, second 2 var
      ...
      // Documentation doesn't say how we should type the information we read from the registers? I'll just follow the return types
      //float N_sq = pow((float)(dwt_read32bitoffsetreg(RX_FINFO_ID, RXPACC)), 2);

      //double first_path_power_dBm = 10 * log10(
      
      //  (pow((float)diag.firstPathAmp1, 2) + pow((float)diag.firstPathAmp2, 2) + pow((float)diag.firstPathAmp3, 2)) / N_sq
      
      //) - PRF;

      // maybe the values I'm getting from diagonstic are not what's useful for these formulas
      // and instead we should be

      // maxGrowthCIR is a growth factor, and isn't actually the CIR power
      // CIR power is obtained from 0x12 CIR_PWR

      //float CIR_power = (float)(dwt_read32bitoffsetreg(RX_FQUAL_ID, CIR_PWR));

      //double RX_power_dBm = 10 * log10( (CIR_power * POW_2_17) / N_sq) - PRF;
      //float NLoS_score = fabs(first_path_power_dBm - RX_power_dBm);
      //bool is_NLoS = NLoS_score > 6.0;
